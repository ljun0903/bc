...
if(isMultipart){
	...
	while(it.hasNext()){
		...
		if(item.isFormField()){ // true는  id, title, content가 넘어온것
			...
			else if(item.getFieldName().equals("pdsnum")){ // pdsnum값을 받는다.
				pdsnum = Integer.parseInt(item.getString("utf-8"));
...
boolean isS = dao.writePds(new PdsDto(id, name, title, content, filename, newfilename, pdsnum));
if(isS){
	%>	
	...
	if(pds == 1 ){
		...
		location.href = "myReviewBbs.jsp?pdsnum=<%=pdsnum%>";
	} else {
		location.href = "myGroupBbs.jsp?pdsnum=<%=pdsnum%>";
	} 
